...
public class ModelGenerator {
    ...
    private Set<TypeSpec> superModels = new HashSet<>();
    ...
    public ModelGenerator(final TableDefinition tableDefinition, final Set<ColumnDefinition> columnDefinitions, final Elements elements, final Types types) {
        ...
        superTypes( classBuilder, tableDefinition.getOriginElement(), tableDefinition.getPackageName(), columnDefinitions );
        ...
        contract(classBuilder, tableDefinition, columnDefinitions);

        generatedModel = classBuilder.build();
    }

    /**
     * When a non relational table (has a origin) has a super class which is a non contract class (no contract annotation)
     * then is the super type converted to a interface.
     */
    ...
    private void generateInterface( TypeElement superClass, String packageName, Set<ColumnDefinition> columnDefinitions ) {
        final TypeSpec.Builder interfaceBuilder = TypeSpec.interfaceBuilder( ClassName.get( packageName, superClass.getSimpleName().toString().concat("Model") ) );
        interfaceBuilder.addModifiers(PUBLIC);
        interfaceBuilder.addJavadoc("Generated by Sidekick (" + new SimpleDateFormat("dd.MM.yyyy - HH:mm", Locale.ENGLISH).format(new Date()) + ")\n");

        final Set<Element> fields = JavaUtils.getMemberFields(superClass, elements, types, false);
        final Set<String> fieldNames = new HashSet<>(fields.size());
        for( Element field : fields ) {
            final String variabelName = field.getSimpleName().toString();
            fieldNames.add( variabelName );
        }
        System.out.println( fieldNames );

        for( ColumnDefinition columnDefinition : columnDefinitions ) {
            if( !fieldNames.contains( columnDefinition.getFieldName() ) ) {
                System.out.println( "Skip " + columnDefinition.getFieldName() );
                continue;
            }
            ...
            interfaceBuilder.addMethod(getMethod.build());
        }

        superTypes( interfaceBuilder, superClass, packageName, columnDefinitions );
        superModels.add( interfaceBuilder.build() );
    }

    private void contract(TypeSpec.Builder classBuilder, TableDefinition tableDefinition, Set<ColumnDefinition> columnDefinitions) {
        ContractGenerator.generate( classBuilder, tableDefinition, columnDefinitions );
    }
    ...
    private void builder(TypeSpec.Builder classBuilder, TableDefinition tableDefinition, Set<ColumnDefinition> columnDefinitions) {
        ...
        final ClassName contract = ClassName.get(tableDefinition.getPackageName(), tableDefinition.getClassName() + "Model.Contract");
        ...
        final ClassName modelBuilderName = ClassName.get(tableDefinition.getPackageName(), tableDefinition.getClassName() + "Model.Builder");
        classBuilder.addMethod( MethodSpec.methodBuilder("builder")
                .addModifiers(PUBLIC, STATIC)
                .returns( modelBuilderName )
                .addStatement( "return new $T()", modelBuilderName )
                .build() );

        classBuilder.addMethod( MethodSpec.methodBuilder("builder")
                .addModifiers(PUBLIC, STATIC)
                .returns( modelBuilderName )
                .addParameter( ParameterSpec.builder( contentValues, "previousValues", FINAL)
                        .addAnnotation(nonNull)
                        .build() )
                .addStatement( "return new $T( $N )", modelBuilderName, "previousValues" )
                .build() );
    }
    ...
    public Set<TypeSpec> getSuperModels() {
        return superModels;
    }
